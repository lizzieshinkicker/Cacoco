...
impl AssetStore {
    ...
    fn create_and_store_texture(
        ...
        println!("{}: '{}' -> Key: '{}' ({}x{})", log_label, name, key, size[0], size[1]);
    ...
    fn load_texture_only(&mut self, ctx: &egui::Context, name: &str, bytes: &[u8]) {
        self.load_image_from_bytes(ctx, name, bytes, egui::TextureOptions::NEAREST, "Loaded Asset");
    }

    pub fn load_smooth_image(&mut self, ctx: &egui::Context, name: &str, bytes: &[u8]) {
        self.load_image_from_bytes(ctx, name, bytes, egui::TextureOptions::LINEAR, "Loaded Smooth Asset");
    }

    pub fn load_rgba(&mut self, ctx: &egui::Context, name: &str, width: u32, height: u32, pixels: &[u8]) {
        if pixels.len() != (width * height * 4) as usize {
            eprintln!("Asset Error: Pixel buffer size mismatch for {}", name);
            return;
        }

        let color_image = egui::ColorImage::from_rgba_unmultiplied([width as _, height as _], pixels);
        self.create_and_store_texture(ctx, name, color_image, egui::TextureOptions::NEAREST, "Loaded RGBA Asset");
    }
    ...
    pub fn load_system_assets(&mut self, ctx: &egui::Context) {
        self.load_reference_image(ctx, "_BADGE_ALLMAP", include_bytes!("../assets/badges/Allmap.png"));
        self.load_reference_image(ctx, "_BADGE_BERSERK", include_bytes!("../assets/badges/Berserk.png"));
        self.load_reference_image(ctx, "_BADGE_BLURSPHERE", include_bytes!("../assets/badges/BlurSphere.png"));
        self.load_reference_image(ctx, "_BADGE_INVULN", include_bytes!("../assets/badges/Invuln.png"));
        self.load_reference_image(ctx, "_BADGE_LITEAMP", include_bytes!("../assets/badges/LiteAmp.png"));
        self.load_reference_image(ctx, "_BADGE_RADSUIT", include_bytes!("../assets/badges/Radsuit.png"));
    }
        ...
        if is_number_font {
            match c_upper {
                '-' => {
                    ...
                    if self.textures.contains_key(&p1) { return p1; }
                    format!("{}-", stem)
                },
                '%' => {
                    ...
                    for v in variants {
                        let p = format!("{}{}", stem, v);
                        if self.textures.contains_key(&p) { return p; }
                    }
                    format!("{}%", stem)
                },
                '0'..='9' => {
