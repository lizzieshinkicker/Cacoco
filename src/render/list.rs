...
fn estimate_element_tree_size(ctx: &RenderContext, element: &ElementWrapper) -> egui::Vec2 {
    let mut size = match &element.data {
        ...
        Element::Number(n) | Element::Percent(n) => {
            ...
            crate::render::text::measure_text_size(ctx, sample_text, &n.font, true)
        }
        Element::String(s) => {
            crate::render::text::measure_text_size(ctx, "Sample Text", &s.font, false)
        }
        Element::Face(_) | Element::FaceBackground(_) => egui::vec2(24.0, 29.0),
        Element::Component(c) => {
            crate::render::text::measure_text_size(ctx, "Sample Text", &c.font, false)
        }
