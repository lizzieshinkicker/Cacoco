...
pub enum PreviewContent {
    Image(String),
    Text { text: String, stem: Option<String>, is_number_font: bool }
}
...
pub fn draw_preview_panel(ui: &mut egui::Ui, assets: &AssetStore, content: PreviewContent) {
    ...
    ui.painter().rect_filled(rect, 4.0, egui::Color32::from_black_alpha(50));
    ui.painter().rect_stroke(rect, 4.0, egui::Stroke::new(1.0, egui::Color32::from_white_alpha(30)), egui::StrokeKind::Inside);
    ...
    match content {
        ...
        PreviewContent::Text { text, stem, is_number_font } => {
            if let Some(s) = stem {
    ...
    if textures.is_empty() {
        ui.painter().text(rect.center(), egui::Align2::CENTER_CENTER, "(No Preview)", egui::FontId::proportional(14.0), egui::Color32::GRAY);
        return;
    }

    let total_width: f32 = textures.iter().map(|t| t.size_vec2().x).sum();
    let max_height: f32 = textures.iter().map(|t| t.size_vec2().y).reduce(f32::max).unwrap_or(10.0);
    ...
    let scale = if raw_scale >= 1.0 { raw_scale.floor() } else { raw_scale };
    ...
    for tex in textures {
        ...
        ui.painter().image(tex.id(), dest_rect, egui::Rect::from_min_max(egui::pos2(0.0,0.0), egui::pos2(1.0,1.0)), egui::Color32::WHITE);
        current_x += size.x;
    }
}